import {getfullPriceEvent, getDateToFullFormat} from '../../utils/event.js';
...
const showFullEventOffers = (offers) => {
...
const createEditEventTemplate = (data) => {
  ...
  const {eventType, offers, price, destination, startDate, endDate, isFavorite} = data;

  const fullOffers = showFullEventOffers(offers);
  ...
  const transferTemplate = createEditEventTransferTemaplte();

  const activityTemplate = createEditEventActivityTemaplte();
...
export default class EventEditAdd extends AbstractView {
  constructor(event = BLANK_EVENT) {
    super();
    this._data = EventEditAdd.parseEventToData(event);
  ...
  _formSubmitHandler(evt) {
    evt.preventDefault();
    this._callback.formSubmit(EventEditAdd.parseEventToData(this._data));
  }

  _favoriteClickHandler(evt) {
    evt.preventDefault();
    this._callback.favoriteClick(); // Вызываем callback
  }
